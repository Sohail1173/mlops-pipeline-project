...
from pipelinesrc.utils.main_utils import load_numpy_array_data,load_object,save_object
...
class ModelTrainer:
    ...
    def get_model_object_and_report(self,train:np.array,test:np.array) ->tuple[object,object]:
        try:
            logging.info("Training RandomForestClassifier with specified paramer")
    ...
    def intiate_model_trainer(self) ->ModelTrainerArtifact:
        ...
        try:
            ...
            if accuracy_score(train_arr[:,-1],trained_model.predict(train_arr[:,:-1])) < self.model_trainer_config.expected_accuracy:
                logging.info("No model found with score about the base score")
                raise Exception("No model found with score about the base score")
